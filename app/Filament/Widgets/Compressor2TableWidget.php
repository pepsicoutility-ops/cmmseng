...
class Compressor2TableWidget extends BaseWidget
{
    ...
    {
        return $table
            ->heading('Compressor 2 - Recent Checklists (Last 7 Days)')
            ->query(
                Compressor2Checklist::query()
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('cooling_delta_t')
                    ->label('Cooling ΔT (°C)')
                    ->state(function (Compressor2Checklist $record): string {
                        $delta = $record->cws_temperature - $record->cwr_temperature;
                        return number_format($delta, 2);
                    })
                    ->color(function (Compressor2Checklist $record): string {
                        $delta = $record->cws_temperature - $record->cwr_temperature;
