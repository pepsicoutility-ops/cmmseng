...
class Chiller1StatsWidget extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        // Today's checklists count
        $checklistsToday = Chiller1Checklist::whereDate('created_at', $today)->count();
        
        // Average evaporator temperature today
        $avgEvapTemp = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('sat_evap_t') ?? 0;
        
        // Average discharge superheat today
        $avgDisSuperheat = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('dis_superheat') ?? 0;
        
        // Average evaporator pressure today
        $avgEvapPressure = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('evap_p') ?? 0;
        
        // Average condenser pressure today
        $avgCondsPressure = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('conds_p') ?? 0;
        
        // Motor amps & volts average today
        $avgMotorAmps = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('motor_amps') ?? 0;
        $avgMotorVolts = Chiller1Checklist::whereDate('created_at', $today)
            ->avg('motor_volts') ?? 0;
        ...
        // Calculate Chiller Health Score (0-100)
        $healthScore = $this->calculateHealthScore($today);

        return [
            Stat::make('Chiller 1 - Checklists Today', $checklistsToday)
            ...
            Stat::make('Chiller 1 Health Score', number_format($healthScore, 0) . '/100')
                ->description($this->getHealthDescription($healthScore))
                ->descriptionIcon('heroicon-o-heart')
                ->color($this->getHealthColor($healthScore))
                ->extraAttributes([
                    'class' => 'cursor-pointer',
                    'wire:click' => '$dispatch("open-health-details", { equipment: "Chiller 1" })',
                ]),
        ];
    }
    ...
    {
        $checklist = Chiller1Checklist::whereDate('created_at', $date)->latest()->first();
        ...
        // 1. Temp/pressure within range (50 points)
        $tempPressureScore = 0;
        
        // Evaporator temp: ideal range 2-8°C
        ...
        $score += $tempPressureScore;

        // 2. Loading within 40-90% (30 points)
        if ($checklist->fla > 0) {
        ...
        // 3. Refrigerant small temp diff within spec (20 points)
        // Cooler temp diff: ideal < 2°C
    ...
    protected function getWeeklyTrend(): array
    {
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $trend[] = Chiller1Checklist::whereDate('created_at', $date)->count();
        }
        return $trend;
    }
    ...
    protected function getHealthDescription(float $score): string
    {
        if ($score >= 80) {
            ...
            return 'Good condition, minor attention needed';
        } elseif ($score >= 40) {
            return 'Fair condition, maintenance required';
        }
