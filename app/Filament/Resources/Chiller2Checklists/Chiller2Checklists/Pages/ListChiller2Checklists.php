...
use App\Filament\Exports\Chiller2ChecklistExporter;
use App\Filament\Imports\Chiller2ChecklistImporter;
...
class ListChiller2Checklists extends ListRecords
{
    ...
    protected function getHeaderActions(): array
    {
        return [
            CreateAction::make(),
            ImportAction::make()
                ->importer(Chiller2ChecklistImporter::class),
            Action::make('downloadCsv')
                ->label('Download CSV')
                ->icon('heroicon-o-arrow-down-tray')
                ->color('success')
                ->action(function () {
                    $checklists = Chiller2Checklist::all();
                    $filename = 'chiller2-checklists-' . now()->format('Y-m-d-His') . '.csv';
